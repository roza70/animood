...
export default function AnimeDetail({ anime: initialAnime, onClose, onAdd, onRate, onNote, isInWatchlist, userNote, userRating }) {
  ...
  const saveEpisodeRating = (epNum) => {
    const val = parseInt(editRatingValue)
    if (isNaN(val) || val < 1 || val > 10) return
    const updated = { ...episodeRatings, [epNum]: val }
    ...
    >
      ...
      >
            ...
            <div style={{ width: "100%", height: "100%", position: "relative" }}>
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "clamp(12px, 3vw, 24px)" }}>
                <h1 style={{
                  ...
                  margin: "0 0 8px 0", textShadow: "0 2px 8px rgba(0,0,0,0.8)",
                }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  {anime?.duration && <span style={{ color: "rgba(255,255,255,0.8)", fontSize: "12px" }}>⏱ {anime.duration}</span>}
                  {anime?.status && (
                    <span style={{
        ...
        <div style={{
          ...
          >{isInWatchlist ? "✓ In Watchlist" : "+ Watchlist"}</motion.button>
              ...
              <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div style={{ display: "flex", gap: "clamp(14px, 3vw, 24px)", flexWrap: "wrap" }}>
                  <img
                    src={anime?.images?.jpg?.large_image_url}
                    alt={anime?.title}
                    style={{ width: "clamp(90px, 12vw, 140px)", borderRadius: "12px", flexShrink: 0, boxShadow: "0 8px 24px rgba(0,0,0,0.3)", alignSelf: "flex-start" }}
                  />
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    ...
                    <p style={{ color: isDark ? "rgba(232,213,245,0.85)" : "#8b0040", fontSize: "clamp(12px, 1.5vw, 14px)", lineHeight: 1.7, margin: 0 }}>
                    ...
                    {characters.map((char, i) => (
                      ...
                      >
                        ...
                        <img
                          ...
                          style={{ width: 56, height: 56, borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: isDark ? "2px solid rgba(200,168,233,0.3)" : "2px solid rgba(233,30,140,0.3)" }}
                        />
                        {/* Character info */}
                        <div style={{ flex: 1 }}>
                          <p style={{ color: isDark ? "#e8d5f5" : "#c2185b", fontSize: "clamp(12px, 1.5vw, 14px)", fontWeight: "700", margin: "0 0 3px 0" }}>
                            {char.character.name}
                          </p>
                          <p style={{ color: isDark ? "#9b7fbf" : "#f06292", fontSize: "11px", margin: "0 0 3px 0" }}>
                            {char.role}
                          </p>
                          {char.voice_actors?.[0] && (
                            <p style={{ color: isDark ? "#7b6090" : "#f48fb1", fontSize: "11px", margin: 0 }}>
                        ...
                        {/* Favorites */}
                        {char.favorites > 0 && (
                          <div style={{ color: "#ffd700", fontSize: "11px", fontWeight: "700", flexShrink: 0 }}>
                        ...
                        {char.voice_actors?.[0]?.person?.images?.jpg?.image_url && (
                          <img
                            ...
                            style={{ width: 40, height: 40, borderRadius: "50%", objectFit: "cover", flexShrink: 0, opacity: 0.7 }}
                          />
                        )}
                      </motion.div>
                    ))}
                    ...
                    {episodes.map((ep, i) => (
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {ep.title_japanese && (
                            <p style={{ color: isDark ? "#7b6090" : "#f48fb1", fontSize: "10px", margin: "2px 0 0 0" }}>
                              {ep.title_japanese}
                            </p>
                          )}
                        </div>
                        ...
                        {/* Score */}
                        {ep.score && (
                          <span style={{ color: "#ffd700", fontSize: "11px", flexShrink: 0 }}>⭐ {ep.score}</span>
                        )}
                        ...
                        {editingEp === ep.mal_id ? (
                          <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                            <input
                              ...
                              onKeyDown={e => {
                                if (e.key === "Enter") saveEpisodeRating(ep.mal_id)
                            ...
                            <motion.button
                              ...
                              onClick={() => saveEpisodeRating(ep.mal_id)}
                              style={{
                            ...
                            <motion.button
                              whileHover={{ scale: 1.1 }}
                              onClick={() => { setEditingEp(null); setEditRatingValue("") }}
                              style={{
                          ...
                          <motion.button
                            whileHover={{ scale: 1.05 }}
                            onClick={() => { setEditingEp(ep.mal_id); setEditRatingValue(episodeRatings[ep.mal_id] || "") }}
                            style={{
            ...
            {activeTab === "my notes" && (
              <motion.div key="notes" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                <textarea
                  ...
                  placeholder="Write your thoughts, feelings, favorite moments..."
                  rows={8}
                  style={{
                ...
                {userNote && (
                  <div style={{
                    marginTop: 20, padding: "clamp(12px, 2vw, 16px)", borderRadius: "14px",
                    background: isDark ? "rgba(200,168,233,0.08)" : "rgba(233,30,140,0.06)",
