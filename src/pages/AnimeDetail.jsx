...
export default function AnimeDetail({ anime: initialAnime, onClose, onAdd, onRate, onNote, onCardClick, isInWatchlist, userNote, userRating }) {
  ...
  useEffect(() => {
    ...
    load()
    loadEpsForId(initialAnime.mal_id, 1)
  }, [initialAnime.mal_id])

  const loadEpsForId = async (id, page) => {
    try {
      setLoadingEps(true)
  ...
  // Build clean trailer URL with autoplay
  ...
  const relationColor = (rel) => {
    if (rel === "Sequel") return isDark ? "#c8a8e9" : "#c06080"
    if (rel === "Prequel") return "#26c6da"
    if (rel === "Side story") return "#ffb74d"
    return isDark ? "#9b7fbf" : "#a06080"
  }
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ width: "100%", maxWidth: "920px", background: bg, borderRadius: "24px", overflow: "hidden", border: isDark ? "1px solid rgba(200,168,233,0.2)" : "1px solid rgba(180,120,140,0.2)", boxShadow: isDark ? "0 0 80px rgba(200,168,233,0.15)" : "0 0 80px rgba(180,120,140,0.15)", marginBottom: "40px" }}
      >
        {/* Hero — fixed height so trailer doesn't shift layout */}
        <div style={{ position: "relative", height: "clamp(200px, 35vw, 360px)", background: "#000", overflow: "hidden" }}>
          {/* Trailer iframe — always rendered when showTrailer, positioned absolute to fill */}
          {trailerUrl && (
            <div style={{
              position: "absolute", inset: 0,
              opacity: showTrailer ? 1 : 0,
              pointerEvents: showTrailer ? "all" : "none",
              transition: "opacity 0.3s",
              zIndex: 2,
            }}>
              <iframe
                src={showTrailer ? trailerUrl : "about:blank"}
          ...
          {/* Poster — always visible, fades when trailer shows */}
          <div style={{
            position: "absolute", inset: 0,
            ...
            {trailerUrl && !showTrailer && (
              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setShowTrailer(true)}
                style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 64, height: 64, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.92)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: "22px", boxShadow: "0 4px 20px rgba(0,0,0,0.5)", zIndex: 3 }}>▶</motion.button>
            )}
        ...
        <div style={{ padding: "clamp(10px, 2vw, 16px) clamp(14px, 3vw, 24px)", display: "flex", gap: 8, flexWrap: "wrap", borderBottom: isDark ? "1px solid rgba(200,168,233,0.1)" : "1px solid rgba(180,120,140,0.15)", alignItems: "center" }}>
          ...
          {/* Trailer toggle button — no layout shift */}
          {trailerUrl && (
          ...
          {relations.length > 0 && (
            <div style={{ width: "100%", marginTop: 6 }}>
              <p style={{ color: isDark ? "#9b7fbf" : "#a06080", fontSize: "11px", fontWeight: "700", margin: "0 0 8px 0", textTransform: "uppercase", letterSpacing: "1px" }}>
                📺 Related Seasons & Entries
              </p>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {relations.map(r => (
                  <motion.button key={r.mal_id} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    onClick={() => handleRelatedClick(r)}
                    style={{ padding: "6px 14px", borderRadius: "12px", border: `1px solid ${relationColor(r.relation)}40`, background: `${relationColor(r.relation)}15`, color: relationColor(r.relation), fontSize: "11px", fontWeight: "600", cursor: "pointer" }}>
                    {r.relation === "Sequel" ? "▶ " : r.relation === "Prequel" ? "◀ " : "• "}{r.name}
                  </motion.button>
                ))}
              </div>
            </div>
                  ...
                  <>
                      ...
                      {episodes.map((ep, i) => (
                        <motion.div key={ep.mal_id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: Math.min(i * 0.02, 0.3) }}
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ color: isDark ? "#e8d5f5" : "#5a3050", fontSize: "clamp(12px, 1.5vw, 13px)", fontWeight: "600", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {ep.title || `Episode ${ep.mal_id}`}
                            </div>
                            {ep.score && <div style={{ color: "#ffd700", fontSize: "10px", marginTop: 2 }}>⭐ {ep.score}</div>}
                          </div>

                          {/* Rate input or button */}
                          {editingEp === ep.mal_id ? (
                            <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                              ...
                              <button onClick={() => saveEpisodeRating(ep.mal_id, editRatingValue)}
                    ...
                    {hasMoreEps && (
                      <div style={{ textAlign: "center", marginTop: 20 }}>
                        <button onClick={() => loadEpsForId(initialAnime.mal_id, episodePage + 1)}
