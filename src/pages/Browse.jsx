...
export default function Browse({ watchlist, onAdd, onRate, onNote, notes, onCardClick }) {
  ...
  return (
    <div style={{
      ...
      {/* Solid color top cover — hides video bleed */}
      <div style={{
        position: "fixed",
      ...
      <div style={{
        padding: "clamp(80px, 12vw, 110px) clamp(16px, 4vw, 48px) 60px",
      }}>
        ...
        >
          ...
          <p style={{ color: isDark ? "#9b7fbf" : "#f06292", fontSize: "14px", margin: 0 }}>
            Search by name or pick a genre to explore 100+ anime
          </p>
        </motion.div>
        ...
        <motion.form
          ...
          style={{ marginBottom: 32, display: "flex", gap: 10, flexWrap: "wrap" }}
        >
          <input
            type="text"
          ...
          <motion.button
            ...
            style={{
              ...
              color: "white", fontSize: "clamp(13px, 1.8vw, 15px)",
                  ...
                  {genreLoading ? (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(clamp(130px, 14vw, 170px), 1fr))", gap: "clamp(12px, 2vw, 20px)" }}>
                      {Array.from({ length: 25 }).map((_, i) => (
                        <motion.div key={i} style={{ aspectRatio: "2/3", borderRadius: "14px", background: isDark ? "rgba(200,168,233,0.1)" : "rgba(233,30,140,0.08)" }}
